...
coin_addresses=($(echo "$output" | grep -oP '(?<=Coin )[a-fA-F0-9x]+' | head -n 5))

# Check if we have at least 5 coin addresses
if [[ ${#coin_addresses[@]} -eq 5 ]]; then
  ...
  echo "Less than 5 coin addresses found. Merge command not executed."
fi
